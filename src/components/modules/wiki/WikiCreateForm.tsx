...
export function WikiCreateForm() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: FormValues) => wikiService.create(data),
    onSuccess: () => {
      toast.success('Item catalogado com sucesso!')
      queryClient.invalidateQueries({ queryKey: ['wiki'] })
      router.push('/wiki')
      router.refresh()
    },
    onError: (error: unknown) => {
      const message = error instanceof Error ? error.message : 'Erro ao catalogar item.'
      toast.error(message)
    },
  })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <Input 
            ...
            className="h-14 bg-zinc-950/60 border-white/10 rounded-2xl focus:border-primary/50 text-white font-bold"
            {...register('name')} 
          />
        ...
        {/* Jogo com AsyncCombobox (Genérico) */}
        <div className="space-y-2">
        ...
        {/* Imagem */}
        <div className="md:col-span-2 space-y-2">
          <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">URL da Imagem</Label>
          <Input 
            placeholder="https://..." 
            className="h-14 bg-zinc-950/60 border-white/10 rounded-2xl focus:border-primary/50 text-white"
            {...register('imageUrl')} 
          />
          {errors.imageUrl && <p className="text-red-500 text-[10px] font-bold uppercase">{errors.imageUrl.message}</p>}
        </div>
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <Textarea 
            ...
            className="min-h-[200px] bg-zinc-950/60 border-white/10 rounded-3xl p-6 focus:border-primary/50 text-white leading-relaxed font-medium resize-none text-sm"
      ...
      <div className="flex gap-4 pt-4">
        <Button
          type="submit"
          disabled={isPending}
          className="flex-1 h-14 bg-primary hover:bg-primary/80 text-black font-black uppercase text-[11px] tracking-widest rounded-2xl shadow-xl shadow-primary/10 transition-all font-sans"
        >
          {isPending ? <Loader2 className="animate-spin" size={18} /> : <><Save size={18} className="mr-2" /> Catalogar Agora</>}
        </Button>
        <Button
          type="button"
          variant="outline"
          onClick={() => router.back()}
          className="h-14 px-8 border-white/10 bg-zinc-950/40 text-zinc-400 hover:text-white rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all font-sans"
        >
